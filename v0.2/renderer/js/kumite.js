...
function addScore(corner, type) {
  ...
  if (match[corner].score >= 8) endMatch(corner, '8-Point Rule');
}
...
function addPenalty(corner, type) {
  ...
  if (match[corner].penalties >= 5) {
    const winner = corner === 'red' ? 'blue' : 'red';
    endMatch(winner, '5 Penalties');
    return;
  }
  updateDisplay();
  broadcast();
  Toast.warning('Penalty', `${corner.toUpperCase()} — ${type}`);
}
...
function timeUp() {
  ...
  else Toast.info('Time Up', 'Match ended in a draw — Encho-sen required');
}
