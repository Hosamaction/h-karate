...
<body>
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('judgeScore', (data) => {
      console.log('Judge score received:', data);
    ...
    socket.on('disconnect', () => {
      ...
      console.log('Judge disconnected, remaining judges:', judges.length);
    });
  });
...
ipcMain.on('match-start', (event, data) => {
  console.log('Broadcasting match start:', data);
  if (displayWindow && !displayWindow.isDestroyed()) {
...
ipcMain.on('judge-score', (event, data) => {
  console.log('Broadcasting judge score:', data);
  if (displayWindow && !displayWindow.isDestroyed()) {
    displayWindow.webContents.send('judge-score', data);
  }
  if (mainWindow && !mainWindow.isDestroyed()) {
...
ipcMain.on('match-end', (event, data) => {
  console.log('Broadcasting match end:', data);
  if (displayWindow && !displayWindow.isDestroyed()) {
...
ipcMain.on('judge-setup', (event, data) => {
  console.log('Setting up web judges with data:', data);
  currentCompetition = data;
  if (io) {
    io.emit('competitionUpdate', data);
  }
